<?php
/**
 * Create Fresh Database - Complete Rebuild
 * This script will DROP the entire database and recreate it from scratch
...
<html>
<head>
    <title>Create Fresh Database</title>
    <style>
...
<body>
<div class='container'>
<h1>Create Fresh Database</h1>
<div class='warning'><strong>⚠ WARNING:</strong> This will COMPLETELY DELETE the database and recreate it from scratch. ALL DATA WILL BE LOST!</div>";
...
try {
    ...
    echo "<div class='info'><strong>Step 2:</strong> Dropping existing database...</div>";
    if ($conn->query("DROP DATABASE IF EXISTS " . DB_NAME)) {
        echo "<div class='success'>✓ Database '" . DB_NAME . "' dropped</div>";
    } else {
        $error = $conn->error;
        if (strpos($error, "doesn't exist") === false) {
            echo "<div class='warning'>⚠ " . $error . " (database may not exist)</div>";
        } else {
            echo "<div class='info'>Database doesn't exist (will be created)</div>";
        }
    }
    
    // Step 3: Create new database
    echo "<div class='info'><strong>Step 3:</strong> Creating new database...</div>";
    $create_db_sql = "CREATE DATABASE " . DB_NAME . " DEFAULT CHARACTER SET utf8mb4 DEFAULT COLLATE utf8mb4_unicode_ci";
    ...
    foreach ($statements as $index => $statement) {
        ...
        if ($conn->query($statement)) {
            ...
            // Ignore "Table already exists" errors (shouldn't happen, but just in case)
    ...
    echo "<tr style='background:#f0f0f0;'><th>Item</th><th>Count</th></tr>";
    ...
    if (count($final_tables) >= 11 && count($errors) == 0) {
        echo "<div class='success'><strong>✓ SUCCESS! Database recreated successfully!</strong></div>";
        ...
        echo "<div class='info'>You can now use your application. All tables have been created fresh.</div>";

...
// Get the payment
$payment_query = $can_delete_any 
    ? "SELECT op.*, p.property_name, p.owner_name FROM owner_payments op INNER JOIN properties p ON op.property_id = p.id WHERE op.id = $payment_id"
    : "SELECT op.*, p.property_name, p.owner_name FROM owner_payments op INNER JOIN properties p ON op.property_id = p.id WHERE op.id = $payment_id AND op.user_id = $user_id";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($property_id) || empty($amount) || empty($payment_month)) {
        ...
        $where_clause = $can_delete_any ? "id = ?" : "id = ? AND user_id = ?";
        
        if ($can_delete_any) {
            $stmt = $conn->prepare("UPDATE owner_payments SET property_id = ?, amount = ?, payment_month = ?, paid_date = ?, payment_method = ?, reference_number = ?, notes = ?, status = ? WHERE id = ?");
            $stmt->bind_param("idssssssi", $property_id, $amount, $payment_month, $paid_date, $payment_method, $reference_number, $notes, $status, $payment_id);
        } else {
            $stmt = $conn->prepare("UPDATE owner_payments SET property_id = ?, amount = ?, payment_month = ?, paid_date = ?, payment_method = ?, reference_number = ?, notes = ?, status = ? WHERE id = ? AND user_id = ?");
            $stmt->bind_param("idssssssii", $property_id, $amount, $payment_month, $paid_date, $payment_method, $reference_number, $notes, $status, $payment_id, $user_id);
        }
...
$page_title = 'Edit Owner Payment';
include '../includes/header.php';
?>

<div class="page-actions">
    <h1>Edit Owner Payment</h1>
        ...
        <form method="POST" action="">
            <div class="form-group">
                ...
                <select id="property_id" name="property_id" required onchange="updateAmountFromProperty()">
                    ...
                    foreach ($properties_array as $property): 
                        $selected = ($property['id'] == $payment['property_id']) ? 'selected' : '';
                    ?>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" id="amount" name="amount" step="0.01" min="0" value="<?php echo htmlspecialchars($payment['amount']); ?>" required>
                ...
                <div class="form-group">
                    ...
                    <input type="month" id="payment_month" name="payment_month" value="<?php echo date('Y-m', strtotime($payment['payment_month'])); ?>" required>
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="date" id="paid_date" name="paid_date" value="<?php echo $payment['paid_date'] ? htmlspecialchars($payment['paid_date']) : ''; ?>">
            ...
            <div class="form-group">
                ...
                <input type="text" id="reference_number" name="reference_number" value="<?php echo htmlspecialchars($payment['reference_number'] ?? ''); ?>" placeholder="Transaction/Check number">
            </div>
            
            <div class="form-group">
                <label for="notes">Notes</label>
                <textarea id="notes" name="notes" rows="3" placeholder="Additional notes about this payment"><?php echo htmlspecialchars($payment['notes'] ?? ''); ?></textarea>
            </div>
